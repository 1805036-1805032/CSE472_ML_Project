...
def evaluateModelAUC(model_name):     
    scores = cross_val_score(model_name, X_train_full, y_train_full,cv=10, scoring='roc_auc')
    return scores.mean(), scores.std() 

def evaluateModelAccuracy(model_name):
    scores = cross_val_score(model_name, X_train_full, y_train_full,cv=10, scoring='accuracy')
    return scores.mean(), scores.std()

def evaluateModelRecall(model_name):
    scores = cross_val_score(model_name, X_train_full, y_train_full,cv=10, scoring='recall')
    return scores.mean(), scores.std()

def evaluateModelPrecision(model_name):
    scores = cross_val_score(model_name, X_train_full, y_train_full,cv=10, scoring='precision')
    return scores.mean(), scores.std()

def evaluateModelF1(model_name):
    scores = cross_val_score(model_name, X_train_full, y_train_full,cv=10, scoring='f1')
    return scores.mean(), scores.std()
...
output_df = pd.DataFrame({"ID":pd.Series(test_folder_names), "Predicted" : pd.Series(y_preds_prob[:,1])})
